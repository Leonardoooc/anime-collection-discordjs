...
module.exports =  {
    data: new SlashCommandBuilder()
        .setName('lu')
        .setDescription('lu')
        .addStringOption(option => option.setName('input').setDescription('name').setRequired(true)),
    async execute(interaction, client) {
        ...
        try {
            ...
            if (checkResult[0].length == 0) return interaction.reply({ content: 'Nenhum personagem encontrado.' });
            let names = [];
            let charID = [];
