...
module.exports =  {
    ...
    async execute(interaction, client) {
        ...
        try {
            ...
            const checkValues = [interaction.user.id, interaction.user.id];
            const checkResults = await connection.query(checkQuery, checkValues);

            if (checkResults[0].length == 0) return interaction.reply({ content: 'Não há nenhum personagem registrado em sua conta.' });
