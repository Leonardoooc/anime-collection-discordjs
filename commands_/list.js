        ...
        try {
            ...
            if (input !== null) {
                ...
                if (queryObject.hasOwnProperty('user')) {
                    ...
                    sql += ` AND userID = ${interaction.user.id}`;
                }
                ...
                if (queryObject.hasOwnProperty('series')) {
                    if (Number.isInteger(parseInt(queryObject.series))) {
                        ...
                        if (searchSeries[0].length == 0) return interaction.reply({ content: 'Nenhum personagem encontrado com essa série.' });
                        let charIDList = [];
                ...
                if (queryObject.hasOwnProperty('name')) {
                    if (Number.isInteger(parseInt(queryObject.name))) {
                        ...
                        let charNameList = [];
                        if (searchNames[0].length == 0) return interaction.reply({ content: 'Nenhum personagem encontrado com esse nome.' });
                ...
                if (queryObject.hasOwnProperty('wishlist')) {
                    ...
                    if (queryObject.wishlist === '') {
                        var sqlWishes = `SELECT * FROM wishlist WHERE userID = ${interaction.user.id}`
                    } else {
                    ...
                    if (wishesSearch[0].length == 0) interaction.reply({ content: 'Nenhum wish encontrado.' });
                    let wishesList = [];
                ...
                if (final[0].length == 0) return interaction.reply({ content: 'Nenhum resultado encontrado para a busca. '});
            } else {
                var sqlSearch = "SELECT * FROM inventory WHERE userID = ? ORDER BY localID";
                var checkValues = [interaction.user.id];
            ...
            if (final[0].length == 0) {
                ...
                while (start < charList.length) {
                    ...
                    const currentPageEmbed = new EmbedBuilder()
                        .setTitle(`${interaction.user.username} - Lista de Personagens`)
